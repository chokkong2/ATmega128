...
void USART1_init (void)               // Dynamixel과 ATmega128은 USART1 사용
{
	...
	DDRA &=0b11111101;            // Direction port 방향 설정
	DDRA |=0b00000010;            // Direction port 방향 설정
}

ISR (USART0_RX_vect)              // PC가 송신하는 Instruction Packet을 수신하기 위한 인터럽트
{
...
ISR (USART1_RX_vect)                             // Dynamixel이 송신하는 Status Packet을 수신하기 위한 인터럽트
{
	 ...
	 if (k == 0)                                 // 문자열을 수신하기전에 문자열을 저장할 배열을 초기화해줌
	 {
		 for (i=0; i<len; i++)
		 {
			 StatusPacket[i] = '\0';
	 ...
	 Tx_MCUtoPC_Char(StatusPacket[k]);
	 k++;
}

void Tx_MCUtoPC (unsigned char* Instrpacket)        // Dynamixel이 송신한 Status Packet을 ATmega128이 PC로 송신
{
	//strcat (InstrPacket,"\n");
...
void Tx_MCUtoPC_Char (unsigned char StatusPacket)        // Dynamixel이 송신한 Status Packet을 ATmega128이 PC로 송신
{
	    while (!(UCSR0A & 0x20));
		UDR0 = StatusPacket;                      // 문자 송신
}

void Tx_MCUtoDyn (unsigned char InstrPacket)         // PC가 송신한 Instruction Packet을 ATmega128이 Dynamixel로 송신
{
	while (!(UCSR1A & 0x20));                    // 송신 버퍼가 비어있는지 확인
	UDR1 = InstrPacket;                      // 문자 송신
